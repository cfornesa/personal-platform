...
2026-06-05 · AI ROUTE ERROR HANDLING · `decryptAiApiKey` in `artifacts/api-server/src/lib/crypto.ts` throws a plain `Error` (not `AiProviderError`) when the stored key cannot be decrypted — silently producing a 500. Both the text-generation and describe-image routes in `artifacts/api-server/src/routes/ai.ts` now wrap this call in an isolated try-catch that returns 409 with a user-facing message. `logger.error` was added at the top of both catch blocks. If a 409 "could not be read" error appears, the fix is to re-save the affected vendor's API key in Admin → AI.
    [Verified from `artifacts/api-server/src/lib/crypto.ts`, `artifacts/api-server/src/routes/ai.ts`, and the resolved 500 on POST /api/ai/describe-image.]

2026-06-05 · AI SETTINGS PERSISTENCE · Two bugs caused the task-preference dropdowns to require multiple saves and hard refreshes: (1) new unsaved profiles had temporary string keys like `"new-1"` whose `Number()` = NaN, so `safePref` silently dropped the preference — fixed by adding `!d.isNew` to the `enabledProfiles` filter in `admin-ai.tsx`; (2) `setQueryData` alone doesn't refetch for late-mounting subscribers — fixed by adding `invalidateQueries` immediately after `setQueryData` in the `onSuccess` handler.
    [Verified from `artifacts/microblog/src/pages/admin/admin-ai.tsx`.]

2026-06-05 · AI ERROR SURFACING · All three describe-image error sites swallowed the server error message behind a hardcoded "Could not generate alt text." fallback. Fixed in `admin-library.tsx`, `FeaturedImagePicker.tsx`, and `RichPostEditor.tsx` to extract `error?.data?.error` (or use `getAiFailureMessage`) before falling back. The `getAiFailureMessage` utility in `artifacts/microblog/src/components/post/ai-error.ts` is the canonical helper for this pattern.
